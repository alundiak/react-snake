...
export function Board({ children }: PropsWithChildren<any>) {
  ...
  return (
    <>
      <div className="wrapper">{boardCore}</div>
      {children}
    </>
  );
}
...
function regeneratePoints(dispatch: any) {
  ...
  setTimeout(() => {
    ...
    dispatch({
      type: "REGENERATE_SNAKE",
      newPosition: generateRandomPoint("snakeHead"),
    });
  }, 3000);
}
...
const isTargetEaten = (boardState: any, newPosition: number[]) => {
  return (
    newPosition[0] === boardState.targetPoint[0] &&
    newPosition[1] === boardState.targetPoint[1]
  );
};

const isItselfEaten = (tail: any, newPosition: number[]) =>
  tail.some((el: number[]) => {
